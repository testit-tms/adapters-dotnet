...
*   **Проверка существования автотеста:** Узнает у Test IT, зарегистрирован ли уже автотест с заданным `ExternalId` (`IsAutotestExist`).
*   **Создание автотестов:** Создает новый автотест в Test IT на основе данных из тестового контейнера и класса (`CreateAutotest`).
*   **Обновление автотестов:** Обновляет существующий автотест в Test IT с полной информацией (`UpdateAutotest(TestContainer, ClassContainer)`) или только со ссылками (`UpdateAutotest(string, List<Link>, string)`).
*   **Отправка результатов тестов:** Сообщает Test IT, как прошел конкретный автотест (успешно, упал, пропущен), с каким сообщением, трассировкой ошибки, шагами и ссылками (`SubmitTestCaseResult`).
*   **Загрузка вложений:** Отправляет файлы (скриншоты, логи) на сервер Test IT и получает их уникальные идентификаторы, которые затем прикрепляются к результатам тестов (`UploadAttachment`).
*   **Связывание с задачами (Work Items):** Сообщает Test IT, что данный автотест покрывает определенные ручные тест-кейсы или задачи (`LinkAutoTestToWorkItems`).
*   **Управление связями с задачами:** Может не только добавлять, но и удалять связи автотеста с задачами (`DeleteAutoTestLinkFromWorkItem`) и получать список текущих связей (`GetWorkItemsLinkedToAutoTest`).
*   **Управление Тест-Ранами:** Создание (`CreateTestRun`), обновление (`UpdateTestRun`) и завершение (`CompleteTestRun`) Тест-Ранов – сессий выполнения тестов.
*   **Получение информации об автотесте:** Запрашивает у Test IT данные конкретного автотеста по его `ExternalId` (`GetAutotestByExternalId`).
...
Вы настраиваете адаптер через [конфигурацию](01_конфигурация_адаптера_.md), используете [атрибуты метаданных](02_Атрибуты_метаданных_test_it_.md), а `AdapterManager`, `IWriter` и `ITmsClient` работают вместе "за кулисами", чтобы ваши результаты попали в Test IT.
