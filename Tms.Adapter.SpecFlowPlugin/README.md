...
| Mode of import type selection when launching autotests (**It's optional**). Default value - false. true — publish each result when ready; false — buffer and publish at end of the run (first result is still InProgress via Sync Storage)                                                                       | importRealtime                    | TMS_IMPORT_REALTIME                        |
